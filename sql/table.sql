...
CREATE TABLE `attendance` (
    ...
    `early_min` smallint  DEFAULT NULL COMMENT 'จำนวนนาทีออก ข้ามวัน',
    `morning` varchar(10) DEFAULT NULL COMMENT 'เวลาเข้า เช้า 06:00 - 10:00 (last)',
    `morning_min` smallint DEFAULT NULL COMMENT 'จำนวนนาทีเข้า เช้า',
    `lunch_out` varchar(10) DEFAULT NULL COMMENT 'เวลาพักเที่ยง 11:00 - 13:30 (first)',
    `lunch_out_min` smallint DEFAULT NULL COMMENT 'จำนวนนาทีพักเที่ยง',
    `lunch_in` varchar(10) DEFAULT NULL COMMENT 'เวลากลับเที่ยง 11:30 - 14:00 (last)',
    `lunch_in_min` smallint DEFAULT NULL COMMENT 'จำนวนนาทีกลับเที่ยง',
    `evening` varchar(10) DEFAULT NULL COMMENT 'เวลาออก เย็น 16:00 - 18:00 (last)',
    `evening_min` smallint DEFAULT NULL COMMENT 'จำนวนนาทีออก เย็น',
    `night` varchar(10) DEFAULT NULL COMMENT 'เวลาออก ค่ำ 19:00 - 24:00 (last)',
    `night_min` smallint DEFAULT NULL COMMENT 'จำนวนนาทีออก ค่ำ',
    ...
    `rawTimes` varchar(60) DEFAULT NULL COMMENT 'เวลาที่สแกน',
    `reason` VARCHAR(255) ,
    ...
    PRIMARY KEY (`comCode`, `scanCode`, `dateAt`),
    CONSTRAINT `attendance_ibfk_1` FOREIGN KEY (`comCode`, `scanCode`) REFERENCES `employee` (`comCode`, `scanCode`)
